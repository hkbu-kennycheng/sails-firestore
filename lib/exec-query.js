...
module.exports = function execQuery(dsEntry, query) {
  ...
  return (function reduceQuery(gq) {
    ...
    try {
      ...
      let limit = parseInt(query.criteria.limit) || docs.length;
      let skip = parseInt(query.criteria.skip) || 0;

      return Promise.resolve(docs.slice(Math.max(skip, 0), Math.min(skip+limit, docs.length)));
    } catch(e) {
      return Promise.reject(e);
    }
  });
